...
tests_cover: clean
	@rm -rf cover
	${PYTHON} setup.py nosetests --cover-branches --with-coverage --cover-html --cover-erase --cover-package=ReOBJ
	$(MAKE) clean
...
docs:
	rm -rf docs/ReOBJ-DOCUMENTATION
	sphinx-apidoc --force --output-dir=docs/source ReOBJ
	cd docs && make html
	cd ..
	$(MAKE) clean
	@echo -e '\n=== finished docs'
...
zip_docs: docs
	rm -rf zipped_docs
	mkdir zipped_docs
	cd docs/ReOBJ-DOCUMENTATION/html && zip -r ../../../zipped_docs/docs_html_ReOBJ.zip *
	cd ..
	$(MAKE) clean
	@echo -e '\n=== finished zip_docs'
